...
func ParseData(data string) (int, error) {
	...
	for _, guard := range guards {
		...
		sort.Slice(guard.Times, func(i, j int) bool {
			if guard.Times[i].Start < guard.Times[j].Start {
				return true
			}
			if guard.Times[i].Start == guard.Times[j].Start {
				return guard.Times[i].End <= guard.Times[j].End
			}
			return false
		})
		...
		if maxCount > maxGuardCount {
			maxGuard = guard
			maxGuardMinute = maxMin
			maxGuardCount = maxCount
		}
	}

	log.Printf("maxGuard: %v\n", maxGuard)

	log.Printf("m: %d, c: %d", maxGuardMinute, maxGuardCount)

	return maxGuardMinute * maxGuard.ID, nil
}
