...
func ParseGame(data string) *Game {
	...
	for y, rowS := range rows {
		...
		for x, r := range rowS {
			if r == Elf || r == Goblin {
				m := &Monster{nextMonsterID, r ,V{x, y}, false, 200, 3}
...
func Iterate(game *Game) {
	originalMonsterPos := map[int]V{}
	for _, m := range game.Monsters {
		originalMonsterPos[m.ID] = m.Pos
	}

	for elfPower := 19; ; elfPower++ {
		// Reset monsters
		st := time.Now()
		log.Printf("running with elfPower: %d\n", elfPower)
		for _, m := range game.Monsters {
			m.Pos = originalMonsterPos[m.ID]
			m.Dead = false
			m.HP = 200
			if m.Kind == Elf {
				m.Power = elfPower
		...
		log.Printf("took %s\n", time.Since(st))
		if !elfDead {
			log.Printf("Required elf power: %d", elfPower)
